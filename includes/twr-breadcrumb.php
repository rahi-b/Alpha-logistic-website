...
if(!function_exists('transto_blog_breadcrumb')){
	function transto_blog_breadcrumb(){?>
				...
				<div class="row">
					<div class="col-md-12">						
						<div class="breadcumb-inner">
							<h1  class="brcrumb_title"><?php esc_html_e('Blog','transto'); ?></h1>		
						</div>	
					</div>
				</div>
			</div>
...
function transto_new_breadcrumbs()
{
global $transto_opt;
$here_text=$home_text=$delimiter="";
if(!empty($transto_opt['transto_current_position'])){
	$here_text        = $transto_opt['transto_current_position'];
}else{
	$here_text        = __( 'You here!','transto' );
}
	
if(!empty($transto_opt['transto_breadhome_textc'])){
	$home_text        = $transto_opt['transto_breadhome_textc'];
}else{
	$home_text        = __( 'Home','transto' );
}
if(!empty($transto_opt['transto_breadbar_textc'])){
	$delimiter        = $transto_opt['transto_breadbar_textc'];
}else{
	$delimiter        = ' - '; 
}
    ...
    if( is_archive() )
    {
        if (    is_category()
            ...
            $breadcrumb_trail = __( 'Author archive for ','transto') .  $before . $queried_object->data->display_name . $after;
    ...
    if ( is_search() ) {
        $breadcrumb_trail = __( 'Search query for: ','transto' ) . $before . get_search_query() . $after;
    }

    /* Handle 404's  */
    if ( is_404() ) {
        $breadcrumb_trail = $before . __( 'Error 404','transto' ) . $after;
    }

    /* Handle paged pages  */
    if ( is_paged() ) {
        $current_page = get_query_var( 'paged' ) ? get_query_var( 'paged' ) : get_query_var( 'page' );
        $page_addon   = $before . sprintf( __( ' ( Page %s )','transto' ), number_format_i18n( $current_page ) ) . $after;
    }
...
if(!function_exists('transto_main_breadcrumb')){
function transto_main_breadcrumb(){
 global $transto_opt;   
 global $post;   
					...
					<div class="col-md-12 txtc  <?php echo esc_attr( $page_text_align );?> <?php echo esc_attr( $page_text_transform );?>">		
						<?php if (!empty($transto_opt['transto_bread_titleh']) && $transto_opt['transto_bread_titleh']==true){?>
							<div class="brpt brptsize">
								<h1 class="brcrumb_title"><?php wp_title(' '); ?></h1>
							</div>
						<?php }else{
							if( $btitle == 'btitles' ) { ?>
							<div class="brpt brptsize">
						...
						if (!empty($transto_opt['transto_bread_menuhide']) && $transto_opt['transto_bread_menuhide']==true){
							...
							<div class="breadcumb-inner">						
								<?php echo transto_new_breadcrumbs(); ?>							
							</div>												
						<?php } ?>
					</div>
				</div>
